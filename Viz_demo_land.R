...
data_df <- separate(data = output_files, col = filename, into = c('junk', 'junk1', 'junk2', 'year'), sep = "/")
...
data_df_plot %>% ggplot(aes(x = XCOORD, y = abs(YCOORD))) + 
...
data_df_L0$pop <- rep(c(data_df_patch_pop$pop), time = 3)
